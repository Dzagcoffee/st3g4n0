...
def main():
    parser = argparse.ArgumentParser(description="LSB Matching стеганография для изображений")
    subparsers = parser.add_subparsers(dest='cmd', required=True)

    enc = subparsers.add_parser('encode', help='Встроить сообщение в изображение')
    enc.add_argument('-i', '--input', required=True, help='Путь к исходному изображению')
    enc.add_argument('-o', '--output', required=True, help='Путь к выходному изображению (лучше PNG)')
    enc.add_argument('-m', '--message', required=True, help='Путь к файлу с сообщением (текст/бинарный)')

    dec = subparsers.add_parser('decode', help='Извлечь сообщение из стего-изображения')
    dec.add_argument('-i', '--input', required=True, help='Путь к стего-изображению')
    dec.add_argument('-o', '--output', required=True, help='Путь к файлу для сохранения результата')
